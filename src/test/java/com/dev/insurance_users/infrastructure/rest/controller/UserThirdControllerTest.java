...
class UserThirdControllerTest {
    ...
    void findThirdUserById_WhenUserExists_ShouldReturnUser() throws Exception {
        ...
        when(userThirdService.findById(userId)).thenReturn(user);
        ...
        verify(userThirdService, times(1)).findById(userId);
        verify(userThirdDtoMapper, times(1)).fromDomainToDto(user);
    }
    ...
    void findThirdUserById_WhenUserDoesNotExist_ShouldReturnNotFound() throws Exception {
        // Given
        Long userId = 99L;
        when(userThirdService.findById(userId)).thenThrow(new RuntimeException("Usuario no encontrado"));
